...
int main()
{
    ...
    lightBuffer.bindBase(static_cast<BufferBindings::Binding>(2));
    ...
    lightingTextureBuffer.bindBase(static_cast<BufferBindings::Binding>(7));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        sp.showReloadShaderGUI({ vs, fs }, "PBR");
        {
            ...
            for (int i = 0; i < mvec.size(); ++i)
            {
                ...
                if (ImGui::SliderFloat4((std::string("Color ") + n.str()).c_str(), value_ptr(mvec.at(i).baseColor), 0.0f, 1.0f))
                {
                ...
                if (ImGui::SliderFloat3((std::string("F0 ") + n.str()).c_str(), value_ptr(mvec.at(i).F0), 0.0f, 0.1f))
                {
        ...
        for (int i = -numSpheres / 2; i < numSpheres / 2 + 1; i++)
        {
            ...
            {
                modelUniform->setContent(translate(sphere->getModelMatrix(), glm::vec3(spacing * i, spacing * j, 0.0f)));
